...
/// dedup (ticket #8, `spec/packet-format.md`). El TTL se decrementa una
...
public enum RelayPolicy {
    /// `nil` si el TTL recibido ya es 0 (agotado, nunca se retransmite).
    ...
    public static func decrementedForRelay(_ packet: BeaconPacket) -> BeaconPacket? {
        guard packet.ttl > 0 else { return nil }
        var relayed = packet
        relayed.ttl -= 1
        return relayed
    }
